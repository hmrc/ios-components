...
extension Components.StatusButtonCardView {

    typealias Model = Components.StatusButtonCardView.ButtonModel

    override class func withPlaceholders() -> UIView {
    ...
    override class func examples() -> [UIView] {
        let buttonDescriptor = ButtonDescriptor(
            style: .primary(true),
            visible: true,
            title: "Learn more and apply",
            alignment: .center
        )
        ...
        let model = Model(
            icon: ExampleImages.coins.image,
            title: "Apply now for Help to Save!",
            body: NSMutableAttributedString.styled(
                style: .body,
                string:
                """
                Help to Save is a government savings scheme where you could get up to £1,200 in tax-free bonuses.

                There are a limited number of accounts available at the moment as part of the trial. Be one of the first to apply for a Help to Save account.
                """
                ).withAlignment(.left),
            iconTintColor: UIColor.Semantic.primaryButtonBackground.raw,
            buttonDescriptor: buttonDescriptor
        )
        // swiftlint:enable line_length

        let buttonDescriptor2 = ButtonDescriptor(
            style: .primary(false),
            visible: true,
            title: ExampleText.LoremIpsum.long.rawValue,
            alignment: .center
        ...
        let modelWithoutBody = Model(
            icon: ExampleImages.info.image,
            title: ExampleText.LoremIpsum.longer.rawValue,
            buttonDescriptor: buttonDescriptor2
        )

        let buttonDescriptor3 = ButtonDescriptor(
            style: .secondary,
            visible: true,
            title: ExampleText.LoremIpsum.long.rawValue,
            alignment: .left
        ...
        let longModel = Model(
            ...
            buttonDescriptor: buttonDescriptor3
        )

        return [
            Components.StatusButtonCardView(model: model),
            Components.StatusButtonCardView(model: modelWithoutBody),
            Components.StatusButtonCardView(model: longModel)
        ]
    }
}
